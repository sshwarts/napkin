...
function resolveRefPath(refStore, token) {
...
function resolveRefs(value, refStore) {
...
function edgePassesMetadataFilter(edge, filter) {
...
function resolveStartNodes(nodes, fromId, fromLabel, onAmbiguous) {
    if (fromId) {
        const found = nodes.find((n) => n.id === fromId);
        if (!found)
            return { error: `from_id "${fromId}" not found.` };
        return { ids: [fromId] };
    }
...
function traverseFromStart(startId, direction, nodesById, outgoingByNode, incomingByNode, filter, maxDepth, maxPaths, maxVisited) {
    const queue = [
        { nodeId: startId, depth: 0, pathNodes: [startId], pathEdges: [] },
    ];
    const visitedNodeDepth = new Map();
...
export function registerTools(server, wss, sessions) {
    const resolveOriginSessionId = (sessionId) => {
    ...
    server.tool("get_canvas_summary", "Returns a compact semantic graph: nodes (id, label, type, optional status/metadata) and edges (id, from, to, optional label). Node types match get_canvas output: box, ellipse, diamond. Floor mode: include_metadata=false and include_status=false returns the cheapest read (id/label/type/from/to only).", {
        ...
        const nodes = structured.nodes.map((node) => {
            ...
            if (includeStatus && node.status !== undefined)
                base.status = node.status;
            if (includeMetadata && node.metadata !== undefined)
                base.metadata = node.metadata;
            return base;
        });
    ...
    server.tool("trace_path", "Traverse semantic graph paths from a start node (from_id or from_label). Supports downstream/upstream/both traversal, branching BFS paths, and optional node/edge filters. Read-only in V1.", {
        ...
        if ("error" in start) {
            return {
                content: [{ type: "text", text: `Error: ${start.error}` }],
                isError: true,
            };
        }
        ...
        for (const startId of start.ids) {
            ...
            if (!startNode)
                continue;
            if (!nodePassesFilter(startNode, filter)) {
                continue;
            }
            const result = traverseFromStart(startId, dir, nodesById, outgoingByNode, incomingByNode, filter, depthLimit, pathLimit, visitedLimit);
    ...
    server.tool("clear_canvas", "Remove all elements from the canvas.", {
    ...
    server.tool("update_canvas", "Add new elements to the canvas. Requires full element definitions. For modifying existing elements, use patch_canvas instead.", {
        ...
        wss.updateCanvas(elements, resolveOriginSessionId(session_id));
        return {
            content: [
                {
                    type: "text",
    ...
    server.tool("style", "Apply style changes to an element. Accepts: color, fill/background, opacity, strokeStyle, strokeWidth.", {
        ...
        const result = styleElement(wss, id, s, resolveOriginSessionId(session_id));
        if ("error" in result) {
            return { content: [{ type: "text", text: `Error: ${result.error}` }], isError: true };
        }
    ...
    server.tool("add_label", "Add a floating text label near an element. Optional metadata is stored as customData (invisible in UI, returned in get_canvas).", {
        ...
        const result = addLabel(wss, text, near_id, metadata, resolveOriginSessionId(session_id));
        if (typeof result === "object" && "error" in result) {
            return { content: [{ type: "text", text: `Error: ${result.error}` }], isError: true };
        }
    ...
    // --- Batch intent tool ---
    server.tool("apply_intents", "Execute an ordered batch of intent/write operations in one MCP call. Supports $ref:name.field substitutions from prior operation outputs. On animate_element boundaries with broadcast_mode='end', pending structural writes are flushed before animation starts.", {
        ...
        const executeOperation = async (op, args) => {
            switch (op.tool) {
                case "add_node": {
                    const label = args.label;
                    if (typeof label !== "string")
                        throw new Error("add_node requires string arg 'label'.");
                    const id = addNode(wss, label, typeof args.shape === "string" ? args.shape : undefined, typeof args.style === "object" && args.style !== null ? args.style : undefined, typeof args.near === "string" ? args.near : undefined, typeof args.metadata === "object" && args.metadata !== null ? args.metadata : undefined, originSessionId);
                    return { id };
                }
                case "connect": {
                ...
                case "resize": {
                    const id = args.id;
                    if (typeof id !== "string")
                        throw new Error("resize requires string arg 'id'.");
                ...
                case "style": {
                    const id = args.id;
                    if (typeof id !== "string")
                        throw new Error("style requires string arg 'id'.");
                ...
                case "add_label": {
                    ...
                    const result = addLabel(wss, text, nearId, typeof args.metadata === "object" && args.metadata !== null ? args.metadata : undefined, originSessionId);
                    if (typeof result === "object" && "error" in result)
                        throw new Error(result.error);
                    return {};
                }
                case "delete_element": {
                    const id = args.id;
                    if (typeof id !== "string")
                        throw new Error("delete_element requires string arg 'id'.");
                ...
                case "patch_canvas": {
                    if (!Array.isArray(args.patches))
                        throw new Error("patch_canvas requires array arg 'patches'.");
                    const patches = args.patches;
                    const notFound = wss.patchCanvas(patches, originSessionId);
                    if (notFound.length > 0)
                        throw new Error(`patch_canvas not found ids: ${notFound.join(", ")}`);
                    return {};
                }
                case "update_canvas": {
                    if (!Array.isArray(args.elements))
                        throw new Error("update_canvas requires array arg 'elements'.");
                    wss.updateCanvas(args.elements, originSessionId);
                    return {};
                }
                case "clear_canvas": {
                ...
                case "add_thought_bubble": {
                    ...
                    if (typeof content !== "string")
                        throw new Error("add_thought_bubble requires string arg 'content'.");
                ...
                case "confirm_thought_bubble": {
                    const id = args.id;
                    if (typeof id !== "string")
                        throw new Error("confirm_thought_bubble requires string arg 'id'.");
                    const ok = confirmThoughtBubble(wss, id);
                    if (!ok)
                        throw new Error(`Thought bubble "${id}" not found.`);
                    return {};
                }
                case "dismiss_thought_bubble": {
                    const id = args.id;
                    if (typeof id !== "string")
                        throw new Error("dismiss_thought_bubble requires string arg 'id'.");
                    const ok = dismissThoughtBubble(wss, id);
                    if (!ok)
                        throw new Error(`Thought bubble "${id}" not found.`);
                    return {};
                ...
                case "animate_element": {
                    ...
                    if (typeof id !== "string")
                        throw new Error("animate_element requires string arg 'id'.");
                    if (typeof duration !== "number")
                        throw new Error("animate_element requires numeric arg 'duration_ms'.");
                    ...
                    const commit = (typeof args.commit === "object" && args.commit !== null)
                        ? args.commit
                        : undefined;
                    const result = await animateElement(wss, id, toObj, duration, easing, commit, originSessionId);
                    if ("error" in result)
                        throw new Error(result.error);
                    return {};
                }
        ...
        for (let i = 0; i < operations.length; i++) {
            ...
            try {
                const resolvedArgs = resolveRefs(op.args, refs);
