...
function getServerInstructions(verbose) {
    if (verbose)
        return SERVER_INSTRUCTIONS_VERBOSE;
...
function buildMcpServer(wss) {
    ...
    server.tool("get_server_instructions", "Return compact or verbose server instructions for agent guidance.", {
        verbose: z.boolean().optional().describe("Return verbose onboarding/debug instructions (default: false)"),
    }, async ({ verbose }) => ({
        content: [{ type: "text", text: getServerInstructions(Boolean(verbose)) }],
    }));
    return server;
}
/**
 ...
 */
async function startStdioServer(wss) {
 ...
 */
async function startHttpServer(wss) {
