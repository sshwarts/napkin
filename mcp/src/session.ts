...
const DEFAULT_PERSIST_PATH = join(
...
export class SessionManager {
  private m_sessions: Map<string, Session> = new Map();
  private m_ttlMs: number;
  private m_persistPath: string;
  private m_cleanupTimer: NodeJS.Timeout | null = null;

  constructor() {
    this.m_ttlMs = parseInt(
      process.env.NAPKIN_SESSION_TTL_MS ?? String(DEFAULT_TTL_MS),
      10
    );
