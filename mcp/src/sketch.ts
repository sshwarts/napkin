...
function renderRectangle(el: ExcalidrawElement, ox: number, oy: number): string {
...
function renderEllipse(el: ExcalidrawElement, ox: number, oy: number): string {
...
function renderDiamond(el: ExcalidrawElement, ox: number, oy: number): string {
  const x = el.x - ox;
  const y = el.y - oy;
...
function renderText(el: ExcalidrawElement, ox: number, oy: number): string {
  if (!el.text) return "";
...
function renderFreedraw(el: ExcalidrawElement, ox: number, oy: number): string {
  const points = el.points as Array<[number, number]> | undefined;
...
function renderLine(el: ExcalidrawElement, ox: number, oy: number): string {
  const points = el.points as Array<[number, number]> | undefined;
  if (!points || points.length < 2) return "";
  const baseX = el.x - ox;
  const baseY = el.y - oy;
...
export function renderElementsToSvg(
  ...
  const toRender: ExcalidrawElement[] = [...targetElements];
...
async function describeWithVision(pngBase64: string, prompt: string): Promise<string> {
  try {
 ...
 */
export async function describeElements(
  ...
  if (targets.length === 0) return { error: "No element IDs provided." };
  ...
  for (const el of targets) {
    ...
    if (el.x < leftMost) leftMost = el.x;
  }
  ...
  const updates: ExcalidrawElement[] = [textEl];
  for (const el of targets) {
    updates.push({
      ...el,
