  ...
  server.tool(
    ...
    async ({ patches }) => {
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: "Error: patches must be a valid JSON array." }],
          isError: true,
        };
      }
      if (!Array.isArray(parsed)) {
        return {
          content: [{ type: "text" as const, text: "Error: patches must be a JSON array." }],
          isError: true,
        };
      }
  ...
  server.tool(
    ...
    "Add new elements to the canvas. Requires full element definitions. For modifying existing elements, use patch_canvas instead.",
    {
      elements: z.string().describe(
        "JSON string of ExcalidrawElement[] to merge into the canvas"
      ),
    },
    async ({ elements }) => {
      let parsed: unknown[];
      ...
      wss.updateCanvas(parsed as import("./types.js").ExcalidrawElement[]);
      return {
        content: [
          {
            type: "text" as const,
